...
function king_view(){
	...
	$kid=kc_get('kid',2);
	$sql="kid,kcompany,kfax,kaddress,ktitle,kname,kemail,kqq,kphone,kcontent,ndate,kreply,nreply";
	...
	if(empty($kid)){
		...
		$s.=$king->htmForm($king->lang->get('feedback/label/company'),$rs['kcompany']);
		...
		$s.=$king->htmForm($king->lang->get('feedback/label/fax'),$rs['kfax']);
		$s.=$king->htmForm($king->lang->get('feedback/label/fax'),$rs['kaddress']);
