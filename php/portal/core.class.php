	...
	if(!$array){
		...
		if($modelid>0){//非内置模型的时候
			$model=$this->infoModel($modelid);
				$resCount=$king->db->getRows_one("select count(*) AS ncount from %s__{$model['modeltable']} where (listid=$listid or listid1=$listid) and nshow=1 and kid1=0;");//显示的主题内容
				$resCountAll=$king->db->getRows_one("select count(*) AS ncount from %s__{$model['modeltable']} where (listid=$listid or listid1=$listid) and kid1=0;");//所有的主题内容
...
*/
	...
	if(is_file(ROOT.$xmlpath)){
		...
		$model=$this->infoModel($info['modelid']);
		if(!$id=$king->db->getRows_one("select * from %s__{$model['modeltable']} where kid={$kid} and (listid={$listid} or listid1={$listid})"))
			return False;

		if($id['kid1']==0){//根文章才记录ncount值
			if(!$res=$king->db->getRows("select kid from %s__{$model['modeltable']} where (listid={$listid} or listid1={$listid}) and nshow=1 and (kid={$kid} or kid1={$kid})"))
				$res=array();
			$count=count($res);
...
public function installmodeltable($modeltable){
	...
	listid1 INT NOT NULL DEFAULT 0,
	ktitle VARCHAR(200) NULL,
...
public function install(){
	...
	ktitle char(255) not null,
	klistname char(200) not null,
	kkeywords char(255) null,
...
public function tag($name,$inner,$ass,$attrib){
	...
	switch(strtolower($type)){
		...
		case 'list':
			...
			$where=" and (listid={$listid} or listid1={$listid}) and kid1=0";
