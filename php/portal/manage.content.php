...
function king_edt(){
	...
	//获取父级listid1,并添加父级隐藏，提交到king__Article表.用于帮助一级栏目调用二级栏目文章并可分页
	$parentData=$king->db->getRows_one('select * from %s_list where listid='.$listid.' limit 1;');
	$listid1 = $parentData["listid1"] ==0 ? $listid : $parentData["listid1"];
	
	$s.=kc_htm_hidden(array('listid'=>$listid,'listid1'=>$listid1,'kid'=>$kid,'kid1'=>$kid1));//这个隐藏域不要放在下面
	foreach($res as $rs){
	...
	if($GLOBALS['ischeck']){
		...
		//更新列表信息
		$king->portal->lastUpdated($listid,'list');
		$king->portal->lastUpdated($listid1,'list');
		//删除缓存重建缓存
		$king->cache->del('portal/list/'.$listid);
		$king->cache->del('portal/list/'.$listid1);
		kc_f_delete($king->config('xmlpath','portal').'/portal/'.$info['modelid'].'/'.wordwrap($kid,1,'/',1).'.xml');
		$id=$king->portal->infoID($listid,$kid);
