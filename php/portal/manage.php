	...
	if($is){
		if($res=$king->db->getRows("select listid,isexist from %s_list where listid1=$listid order by norder desc,listid desc;")){
			...
			foreach($res as $rs){
				...
				$js.="\$('#tr_{$info['listid']}').remove();$('#tr_{$listid}').after($str);\$.kc_ready('#tr_{$info['listid']}');";
...
//		$s.=$king->htmForm($_checkbox.$king->lang->get('system/common/content'),null,array(),'Content');
		...
		$s.="<script>";
		$s.="function seiscontent(){var obj=\$('#iscontent');if(obj.prop('checked')==true){\$('#iscontent1').show();}else{\$('#iscontent1').hide()}};seiscontent();";
		$s.="</script>";
