...
$(document).ready(function() {
    ...
    $.kc_ready();
    //用ESC关闭ajax窗口
    $($.browser.msie ? document: window).keydown(function(e) {
        $.kc_keydown(e.keyCode)
    });

    $('.k_help').reset_href().click(function() {
        $.kc_help_click(this)
    }).blur(function() {
        $('#k_help_Fly').fadeOut(300)
    });

    //右键
    $('.k_table_list').bind("contextmenu",
    function(e) {
        //在这里书写代码，构建个性右键化菜单
        //
...
*/
        if ($('#k_list_right_Fly').length > 0) {
            $('#k_list_right_Fly').css('top', e.pageY - 1).css('left', e.pageX - 1).show();
            return false;
        } else {
            return true;
        }
...
*/
    ...
    $.kc_ready = function(pre) {
        ...
        $(pre + '.k_ajax').reset_href().click(function() {
            $.kc_ajax($(this).attr('rel'))
        });
        $(pre + '.k_goto').reset_href().click(function() {
            $.kc_goto($(this).attr('rel'));
            return false;
        });
        /*不要删除return false,否则IE6下出现无法跳转的bug*/
        //$(pre+'a.k_ajax,a.k_help,a.k_setvalue,a.k_updown').attr('href','javascript:;');//重置k_ajax链接为空->javascript:;
        /*
        $(pre + '.k_aselect').on("click",function() {
            $("#k_form_list input[name=list]").prop('checked', true)
        }); //全选
        $(pre + '.k_rselect').on("click",function() {
            $("#k_form_list input[name=list]").each(function(i) { //反选
                $(this).prop('checked', !$(this).prop('checked'))
            })
        });
        */
        $(pre + '.k_setvalue').reset_href().click(function() {
            $.kc_setvalue_click(this)
        }).blur(function() {
            $('#k_setvalue_Fly').fadeOut(300)
        });

        $(pre + '.k_updown').reset_href().click(function() {
            $.kc_updown_click(this)
        }).blur(function() {
            $('#k_updown_Fly').fadeOut(300)
        });

        $(pre + '.k_cmd').click(function() {
            $.kc_cmd_click(this)
        }).blur(function() {
            $('#k_cmd_Fly').fadeOut(300)
        });

        $(pre + '.k_calendar').click(function() {
            $.kc_calendar(this)
        }).blur(function() {
            $('#k_calendar_Fly').fadeOut(300)
        });

        $(pre + '.k_color').click(function() {
            $.kc_color(this)
        }).blur(function() {
            $('#k_color_Fly').fadeOut(300)
        });

        $(pre + '.k_float').mousedown(function(e) {
            $.kc_float(this, e)
        }).fadeTo(50, 1.0);

        $(pre + '.k_verify').click(function() {
            $.kc_verify(this)
        }).focus(function() {
            $.kc_verify(this)
        });

        $(pre + "*[class^='k_user_']").reset_href().click(function() {
            $.kc_user_action(this, pre)
        });
    ...
    $.kc_help_click = function(obj) {
        ...
        $('#k_help_Fly').css('top', o.top + top).css('left', o.left + left).css('zIndex', 1100).fadeIn(100).click(function() {
            $(this).fadeOut(300)
        });
    }

    $.kc_progress = function(id, title, text, prop) {
        $('#' + id).children('label').html(title).end().children('var').html(text).end().find('>span>em').width(prop);
    }
...
*/
    ...
    $.kc_user_action = function(obj, pre) {
        ...
        $.kc_ajax({
            URL: $.kc_root() + 'user/index.php',
            METHOD: 'GET',
            CMD: s.substr(7, s.length - 7),
            IS: (pre == '' ? 0 : 1)
        });
    }
    ...
    $.kc_ajax = function(Rel) { //ajax调用
        ...
        $.ajax({
            ...
            error: function() {
                if (id == 'k_ajax') {
                    $.kc_ajax_show("{title:'Error!',main:'<p class=\"k_err\">'+$.kc_lang('timeout')+'</p>',but:'<a href=\"javascript:;\" class=\"k_close\">'+$.kc_lang('enter')+'</a>',width:320,height:100}");
                } //else{alert($.kc_lang('timeout'))}//k_msg出错?
            },
            success: function(s) {
				$.kc_ajax_text = "";
                if (id == 'k_ajax') {
...
*/
    $.kc_list = function(id, tit, link, isid, isgray, ico, space, first, after) {
        ...
        if (link != '' && link != undefined && link.substr(0, 1) != '{') {
            ...
            if (link.substr(0, 1) == '{' && link.substr(link.length - 1, 1) == '}') tit = '<a href="javascript:;" class="k_ajax" rel="' + link + '">' + tit + '</a>';
        }
...
*/
    $.kc_icon = function(clas, alt, id) {
        ...
        return $.strf('<img%s class="os%s" alt="%s" title="%s" src="../system/images/white.gif"/>', hid, clas ? ' ' + clas: '', alt, alt);
    };
...
*/
    ...
    $.kc_calendar_show = function(d, id) {
        ...
        $('#k_calendar_Fly td:parent').hover(function() {
            $(this).addClass('hover')
        },
        function() {
