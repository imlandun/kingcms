...
public function tag($name,$inner,$ass,$attrib){
	switch($name){
		case 'skin.mainmenueditor': return $this->tag_mainmenueditor();
...
/**
	...
	@return string
*/
private function tag_mainmenueditor(){
	global $king;
	//$cachepath='system/mainmenueditor/'.$king->admin['adminid'].'/'.$king->admin['adminlanguage'];
	...
	if(count($_array)>1){
		...
		foreach($_array as $_value){
			...
			$str.=kc_getlang($_value).'</a>';
			if($i<count($_array)-1){
				$str.='<span class="sp"></span>';	
			}
			$i++;
		}
	}
	$s = "<div id='k_menu_editor'><span class='k_menu_kl'>【编辑器选择：</span>";
	$s.= $str;
	$s.="<span class='k_menu_kr'>】</span></div>";
...
private function tag_mainMenu(){
	...
	/*
	//把editor单独放到了到了tag_mainmenueditor()里面
	//editor
	$_array=kc_f_getdir('system/editor/','dir');
	if(count($_array)>1){
		$i=0;
		...
		if($king->acc('#open_setting')){
			...
			$s.='<li class="k_menu_i">
				...
				<a href="javascript:;">'.$king->lang->get('system/common/tools').'</a>
					...
					if($king->acc('#conn'))
						//隐藏“后台-工具-跨站通信”
						$s.='<li class="hr" style="display:none"><a href="../system/manage.php?action=conn"><img src="../system/images/white.gif" class="os r5"/>'.$king->lang->get('system/title/conn').'</a></li>';
					if($king->acc('#lnkclass'))
						//隐藏“后台-工具-快捷方式管理”
						$s.='<li style="display:none"><a href="../system/manage.php?action=lnk"><img src="../system/images/white.gif" class="os r1"/>'.$king->lang->get('system/title/lnk').'</a></li>';
					if($king->acc('#upfile'))
	...
	*/
		...
		if($king->acc('#open_help')){
			//隐藏后台管理 左侧帮助菜单
			$s.='
			<li class="k_menu_i" style="display:none;">
